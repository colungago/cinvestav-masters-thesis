...
\begin{equation}
\widehat{\text{Var}}(y(r_j)) = \text{Trace}[L^{T}(r_j)C^{-1}L(r_j)]^{-1}.
\end{equation}

Por lo general, la señal medida está contaminada por ruido no uniformemente distribuido y por lo tanto, la varianza de la señal estimada se normaliza con la varianza del ruido proyectado $C_n$ calculada sobre parte de las mediciones en estado basal o en reposo (\emph{baseline noise}). Esta estimación normalizada es denomida Índice de Actividad Neural (NAI del inglés Neuronal Activity Index) \cite{VanVeen1997} y puede expresarse como

\begin{equation}
\text{NAI}(r_j) = \frac{\text{Trace}\left\{[L^{T}(r_j)C^{-1}L(r_j)]^{-1}\right\}}{\text{Trace}\left\{[L^{T}(r_j)C_n^{-1}L(r_j)]^{-1}\right\}}
\end{equation}
