...
\section{Análisis Estadístico del Error de Localización de las Fuentes de Actividad Neuronal}
\label{sec:methodology:statistical}

Para la cuantización del error de localización se procedió obtener la posición de magnitud máxima de los mapas de PNAI generados por el kernel de proyección para cada uno de los 90,000 conjuntos de datos. Estos fueron separados y analizados por cada BSCR, dipolo, y nivel de SNR. La posición de magnitud máxima fue comparada con la posición real de los dipolos de corriente, y se estandarizó con la distancia media y la desviación estándar entre las caras de los triángulos de la malla de la corteza cerebral. 

Entrando en detalle, la posición de magnitud máxima fue obtenida al implementar el kernel de proyección correspondiente a las mediciones de EEG en el instante de 397 ms \TODO{CONFIRM WITH LAB DATA}, el cual es el mismo punto en el que se encuentra la magnitud máxima en la componente $Y$ \TODO{confirmar componente} de la función del dipolo con respecto al tiempo. La posición de magnitud máxima para cada medición de EEG fue obtenida en coordenadas SCS y agrupadas en sus respectivos dipolos, BSCR, y niveles de SNR teniendo como resultado 900 conjuntos de 100 posiciones de magnitud máxima; para poder ser comparados con las posiciones reales de los dipolos de corriente, se calculó el percentil




y fue comparada con la posición real de los dipolos de corriente, siendo el error la distancia euclidiana entre la posición real y la posición de magnitud máxima de la simulación. La distancia media y la desviación estándar entre las caras de los triángulos de la malla de la corteza cerebral fueron obtenidas con el fin de estandarizar el error de localización con respecto a la resolución de la malla. 

El análisis estadístico del error de localización de las fuentes de actividad neuronal se realizó con el fin de obtener un estimador no sesgado de la razón de conductividad cerebro-cráneo. Para ello, se utilizó la frontera de Cramér-Rao (CRB, del inglés \emph{Cramér-Rao bound}) que es una cota inferior para la varianza de un estimador no sesgado. La CRB se calcula a partir de la matriz de ganancia y la matriz de covarianza de las mediciones de EEG, y nos permite obtener la varianza de un estimador no sesgado de la posición de las fuentes de actividad neuronal.


Posicion de magnitud máxima con posicion realista

calculo de distancia media y desviación estandar entre caras de los triangulos de la malla de la corteza cerebral
